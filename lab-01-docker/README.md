...
- **Файловая система:** Скрипт - ленивый хакер. Он запустил бинарник прямо на хосте, и тот нагло палит всю папку `/workspace`. Настоящий Docker устраивает процессу тотальную амнезию через `chroot`/`pivot_root`, наглухо запирая его в изолированном, чистом `rootfs` образа.
- **Сетевой вакуум:** Кокон просто паразитирует на портах текущей УБУНТУШКИ. Оригинальный Docker выстраивает отдельную цифровую вселенную: выделяет контейнеру свой айпи, поднимает мосты `docker0`, виртуальные пары `veth` и на лету рулит трафиком через правила `iptables`.
- **Слои (UnionFS):** Любые правки пишутся прямо на живую ФС хоста. Docker разворачивает магию слоёв `OverlayFS` - база образа лежит в бетонном `Read-Only`, а процесс может лишь царапать файлы во временном верхнем слое экономя мегабайты.
- **Жизненный цикл:** Cервер держится на соплях и честном слове конструкции `trap EXIT`. Докером же заправляет суровый промышленный демон `dockerd` через `containerd`, который в соло тащит автоперезапуски `--restart`, сбор логов и сложную оркестрацию.

Итог вскрытия:
Корона оказалась бутафорской, а монстр лишь бледной копией. Cкрипт совпал с Докером только в одном: под капотом они оба дергают одинаковые низкоуровневые ручки ядр. Но пока мой Франки едва дышит в своей одиночной камере, настоящий Docker разворачивает вокруг процесса колоссальную инфраструктурную обвязку!
