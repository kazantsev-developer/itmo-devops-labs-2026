...
Так вот, `gVisor` полностью переигрывает эту схему. Это по сути своей песочница-переводчик от Google. Она создаёт внутри контейнера собственное виртуальное мини-ядро `Sentry`, написанное кстати на гохе. Когда мой сервис делает системный вызов, `gVisor` перехватывает его, фильтрует и обрабатывает сам, вообще не пуская процесс к реальному ядру хоста.
